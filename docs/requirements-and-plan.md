...
| Versioning | `TokkDb.Pages.Versions` (the version store), `TokkDb.Documents.Delta` (the delta library), and `DbEntities`/`TokkDbConnection` members for history, restore, purge, erase and verification | Versioning (C2.4) landed through `docs/versioning-requirements-and-plan.md`: every write of a `KeepVersions` collection — the default for new collections since step 5.2 — records a version as a delta or keyframe in a history collection, readable by version and by moment through the current schema, restorable, purgeable and erasable within V-17's boundary; the assistant's adapter follows in Phase 9 of that plan |
...
| 3 | Built-in DBMS versioning with delta and version tree (C2.4) | Yes — delta history with keyframes (V-1), a version forest with restore and branches (V-9), purge (V-15) and erase (V-17); the model-based test of NF-6 and the guarantee tests of §6 in `docs/versioning-requirements-and-plan.md` | VR-1…VR-13 closed by that plan's §11; VR-9 (related restore) at its step 6.3 | Closed |
...
| Versioning deferred (D-5) slips past the evaluation | Versioning, the central feature, has no implementation behind it at the point results are produced | Retired: D-5 was settled by V-1 of `docs/versioning-requirements-and-plan.md` (delta with keyframes, *k* = 8 and ratio 0.5 measured in `docs/benchmarks.md`), and versioning landed on the compatible format VR-11, VR-12 and ST-9 kept — the step 0.2 fixture opens and switches on under it (NF-7) |
