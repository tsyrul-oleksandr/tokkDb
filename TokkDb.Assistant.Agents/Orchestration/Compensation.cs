...
/// </summary>
public sealed record Blocker(RecordReference Record, string Reason, RecordReference? Other = null)
{
...
/// </summary>
public sealed class CompensationRefusedException(IReadOnlyList<Blocker> blockers, bool atReplay)
...
/// <summary>
/// Compensation of one request's record changes, as AJ-4 of the versioning plan states it and as
/// step 4.7 of the assistant plan adopts it: two passes inside one unit of work, over
/// <see cref="IStorage"/> and nothing else, so that it means the same over both backends.
///
/// <b>Lifted unchanged from the test-local helper the versioning plan's step 9.3 wrote</b>
/// (<c>TokkDb.Assistant.Tests/Compensation.cs</c>), which still runs its own tests against it.
/// Structural changes are not here - their inverse is in the journal's payload (AG-11e) and is
/// replayed at its position by <see cref="UndoFlow"/>.
/// </summary>
public static class Compensation
{
    ...
    public static IReadOnlyList<Blocker> Validate(IStorage storage, IReadOnlyList<DataChange> included)
    {
        ...
        foreach (var (record, changes) in byRecord)
        {
            ...
            var fields = new Dictionary<string, object?>(current?.Fields ?? new Dictionary<string, object?>(), Names.Comparer);
        ...
        foreach (var (record, end) in endStates)
        {
            ...
            if (end.Deleted)
            {
                ...
                foreach (var relation in relations.Where(relation => Names.Same(relation.ToCollection, definition.Name)))
                {
            ...
            foreach (var column in definition.Columns.Where(static column => column.Unique))
            {
                ...
                blockers.Add(new Blocker(record, $"'{column.Name}' would be {Values.Show(value)}, which {reference} holds", reference));
            }

            // References that would not resolve.
            foreach (var relation in relations.Where(relation => Names.Same(relation.FromCollection, definition.Name)))
            {
                ...
                var restoredHolder = endStates.Any(entry =>
                    Names.Same(entry.Key.CollectionName, relation.ToCollection) && !entry.Value.Deleted
                ...
                if (reference is { } target && endStates.TryGetValue(target, out var targetEnd)
                    ...
                    && !restoredHolder)
                {
                    blockers.Add(new Blocker(record, $"'{relation.FromColumn}' would refer to {Values.Show(value)}, which the undo takes away from {target}", target));
                }
                else if (reference is null && !restoredHolder)
                {
                    blockers.Add(new Blocker(record, $"'{relation.FromColumn}' would refer to {Values.Show(value)}, which nothing in '{relation.ToCollection}' holds"));
                }
            }
        }
    ...
    /// <summary>Compared the way the index that enforces the rule compares: text folded, everything else as itself.</summary>
    private static bool SameValue(IStorage storage, string collectionName, string columnName, object? left, object? right)
    {
        ...
        if (type is ColumnType.Text && left is string l && right is string r)
        {
            return string.Equals(Fold(l), Fold(r), StringComparison.Ordinal);
        }

        return Equals(left, right);
    }

    /// <summary>The contract's text comparison (<c>TextComparison</c>): accents dropped, case folded, the first 128 characters.</summary>
    private static string Fold(string text)
    {
        ...
        {
            if (System.Globalization.CharUnicodeInfo.GetUnicodeCategory(character) is System.Globalization.UnicodeCategory.NonSpacingMark) continue;
            folded.Append(character);
        }

        var recomposed = folded.ToString().Normalize(System.Text.NormalizationForm.FormC).ToUpperInvariant();
        return recomposed.Length <= 128 ? recomposed : recomposed[..128];
    }
