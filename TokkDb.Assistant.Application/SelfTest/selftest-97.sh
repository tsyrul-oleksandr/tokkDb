...
mkdir -p "$C"
for f in conferences-2025.csv conferences-bad.csv empty.csv trips-2025.csv; do cp "/Users/ts/Student/db/tokkDb/TokkDb.Assistant.Application/SelfTest/$f" "$C/$f"; done
...
sed "s#\$C#$C#g" /Users/ts/Student/db/tokkDb/TokkDb.Assistant.Application/SelfTest/selftest-97a.txt > "$C/selftest.txt"
open -n "$APP" || exit 1
i=0
...
sed "s#\$C#$C#g" /Users/ts/Student/db/tokkDb/TokkDb.Assistant.Application/SelfTest/selftest-97b.txt > "$C/selftest.txt"
rm -f "$C/selftest.log"
open -n "$APP" || exit 1
i=0
